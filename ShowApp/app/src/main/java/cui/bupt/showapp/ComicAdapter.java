...
public class ComicAdapter extends ArrayAdapter<Comic> {
    ...
    public ComicAdapter(Context context, int resource, List<Comic> objects) { super(context, resource, objects);
...
// 创建布局
        ...
        View oneComicView = LayoutInflater.from(getContext()).inflate(R.layout.comic_item, parent, false);

// 获取 ImageView 和 TextView

        ImageView imageView = (ImageView) oneComicView.findViewById(R.id.comic_small_imageView);
        TextView textView = (TextView) oneComicView.findViewById(R.id.comic_name_textView);
...
// 往 Intent 中传入 Teacher 相关的数据，供 TeacherDetailActivity使用
            ...
            intent.putExtra("teacher_image", comic.getImageId()); intent.putExtra("teacher_desc", comic.getDesc());
